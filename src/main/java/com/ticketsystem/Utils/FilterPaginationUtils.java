...
public class FilterPaginationUtils {

    public static <P, PD, C, CD, R> Mono<PageResponse<R>> fetch(
            ...
            Integer pageSize,
            Sort sort,
            Function<P, PD> primaryMapper,
            Function<P, Mono<List<C>>> fetchChildren,
            Function<C, CD> childMapper,
            BiFunction<PD, List<CD>, R> resultMapper
    ) {
        ...
        Flux<R> contentFlux = template
                ...
                .all()
                .flatMap(primary -> {
                    PD primaryDto = primaryMapper.apply(primary);
                    return fetchChildren.apply(primary)
                            .defaultIfEmpty(List.of())
                            .map(children -> {
                                List<CD> childDtos = children.stream()
                                        .map(childMapper)
                                        .toList();
                                return resultMapper.apply(primaryDto, childDtos);
                            });
                });
    ...
    public static <P, R> Mono<PageResponse<R>> fetchSimple(
            R2dbcEntityTemplate template,
            Class<P> primaryClass,
            Criteria criteria,
            Integer pageNumber,
    ...
    public static <P, C, R> Mono<PageResponse<R>> fetchRaw(
            R2dbcEntityTemplate template,
            Class<P> primaryClass,
            Criteria criteria,
            Integer pageNumber,
